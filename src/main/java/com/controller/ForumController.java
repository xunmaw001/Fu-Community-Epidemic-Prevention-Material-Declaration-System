    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        Wrapper<ForumEntity> queryWrapper = new EntityWrapper<ForumEntity>()
            ...
            .eq("forum_phone", forum.getForumPhone())
...
//                            forumEntity.setForumPhone(data.get(0));                    //图片 要改的
...
//                            forumEntity.setCreateTime(date);//时间
                            ...
                            //把要查询是否重复的字段放入map中
                                //图片
                                if(seachFields.containsKey("forumPhone")){
                                    List<String> forumPhone = seachFields.get("forumPhone");
                                    forumPhone.add(data.get(0));//要改的
                                }else{
                                    List<String> forumPhone = new ArrayList<>();
                                    forumPhone.add(data.get(0));//要改的
                                    seachFields.put("forumPhone",forumPhone);
                                }
                        }

                        //查询是否重复
                         //图片
                        List<ForumEntity> forumEntities_forumPhone = forumService.selectList(new EntityWrapper<ForumEntity>().in("forum_phone", seachFields.get("forumPhone")));
                        if(forumEntities_forumPhone.size() >0 ){
                            ArrayList<String> repeatFields = new ArrayList<>();
                            for(ForumEntity s:forumEntities_forumPhone){
                                repeatFields.add(s.getForumPhone());
                            }
                            return R.error(511,"数据库的该表中的 [图片] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
