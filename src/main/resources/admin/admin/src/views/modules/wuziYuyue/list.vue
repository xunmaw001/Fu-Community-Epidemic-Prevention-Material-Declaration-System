<template>
    <div class="main-content">
        <el-dialog title="审核" :visible.sync="wuziYuyueYesnoTypesVisible">
            <el-form :model="form">
                <input type="hidden" v-model="form.id">
                <el-form-item label="审核" >
                    <el-select v-model="form.wuziYuyueYesnoTypes" placeholder="请选择申请状态">
                        <el-option label="已同意" value="2"></el-option>
                        <el-option label="已拒绝" value="3"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="申请结果">
                    <el-input type="textarea" v-model="form.wuziYuyueYesnoText" placeholder="申请结果"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="wuziYuyueYesnoTypesVisible = false">取 消</el-button>
                <el-button type="primary" @click="wuziYuyueYesnoTypesShenhe">审 核</el-button>
            </div>
        </el-dialog>
        <!-- 条件查询 -->
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '申请状态' : ''">
                        <el-select v-model="searchForm.wuziYuyueYesnoTypes" placeholder="请选择申请状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in wuziYuyueYesnoTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('wuziYuyue','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/shequfywzshenbao/upload/wuziYuyueMuBan.xls"
                        >批量导入物资申请数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('wuziYuyue','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="wuziYuyueUploadSuccess"
                                :on-error="wuziYuyueUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('wuziYuyue','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "wuziYuyue.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="wuziYuyueNumber"
                                      header-align="center"
                                      label="申请数量">
                        <template slot-scope="scope">
                            {{scope.row.wuziYuyueNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="wuziYuyueYesnoTypes"
                                      header-align="center"
                                      label="申请状态">
                        <template slot-scope="scope">
                            {{scope.row.wuziYuyueYesnoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="wuziYuyueYesnoText"
                                      header-align="center"
                                      label="申请结果">
                        <template slot-scope="scope">
                            <span v-if="scope.row.wuziYuyueYesnoText != null &&scope.row.wuziYuyueYesnoText.length>10">
                                {{scope.row.wuziYuyueYesnoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.wuziYuyueYesnoText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('wuziYuyue','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('wuziYuyue','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('wuziYuyue','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('wuziYuyue','审核') && scope.row.wuziYuyueYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              wuziYuyueYesnoTypesSelectSearch : [],
            form:{
                id : null,
                wuziId : null,
                yonghuId : null,
                wuziYuyueNumber : null,
                wuziYuyueYesnoTypes : null,
                wuziYuyueYesnoText : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            wuziYuyueYesnoTypesVisible : false,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '电子邮箱': 'yonghuEmail',
                //本表字段
                     '申请数量': "wuziYuyueNumber",
                     '申请状态': "wuziYuyueYesnoValue",
                     '申请结果': "wuziYuyueYesnoText",
                     '申请时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'wuzi_yuyue',//当前表表名,
                        sumColum : 'wuzi_yuyue_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'wuzi_yuyue_name',//分组字符串字段
                        // types : 'wuzi_yuyue_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.wuziYuyueYesnoTypes!= '' && this.searchForm.wuziYuyueYesnoTypes!= undefined) {
                    params['wuziYuyueYesnoTypes'] = this.searchForm.wuziYuyueYesnoTypes
                }
                        
                params['wuziYuyueDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=wuzi_yuyue_yesno_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.wuziYuyueYesnoTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            wuziYuyueUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "wuziYuyue/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuziYuyueUploadError(data){
                ...
                _this.form.id = null;
                _this.form.id = id;
                            _this.form.wuziYuyueYesnoText = null;
                _this.form.wuziYuyueYesnoTypes = "请选择申请状态";
                _this.wuziYuyueYesnoTypesVisible = true;
            },

            wuziYuyueYesnoTypesShenhe() {
                let _this = this;
                if(_this.form.wuziYuyueYesnoTypes == "请选择申请状态"){
                    _this.$message.error("请选择申请状态");return false;
                }
                if(_this.form.wuziYuyueYesnoText == null || _this.form.wuziYuyueYesnoText == ""){
                    _this.$message.error("请输入申请结果");return false;
                }
                this.$http({
                    url:`wuziYuyue/shenhe`,
                    method: "post",
                    data: _this.form
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.$message({
                            ...
                            duration: 1500,
                            onClose: () => {
                                _this.wuziYuyueYesnoTypesVisible = false;
                                _this.search();
                            }
                        });
                    } else {
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "wuziYuyue/update",
                        method: "post",
                        data: {
                            id:id,
//                            wuziYuyueTypes:wuziYuyueTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
