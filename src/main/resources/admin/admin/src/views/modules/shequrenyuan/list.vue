        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('shequrenyuan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/shequfywzshenbao/upload/shequrenyuanMuBan.xls"
                        >批量导入社区人员数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shequrenyuan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="shequrenyuanUploadSuccess"
                                :on-error="shequrenyuanUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('shequrenyuan','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入社区人员数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('shequrenyuan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "shequrenyuan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shequrenyuanEmail"
                                   header-align="center"
                                   label="电子邮箱">
                        <template slot-scope="scope">
                            {{scope.row.shequrenyuanEmail}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shequrenyuanAddress"
                                   header-align="center"
                                   label="住址">
                        <template slot-scope="scope">
                            {{scope.row.shequrenyuanAddress}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('shequrenyuan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('shequrenyuan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('shequrenyuan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                id : null,
                shequrenyuanName : null,
                sexTypes : null,
                shequrenyuanPhoto : null,
                shequrenyuanPhone : null,
                shequrenyuanEmail : null,
                shequrenyuanAddress : null,
                shequrenyuanDelete : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'shequrenyuan',//当前表表名,
                        sumColum : 'shequrenyuan_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'shequrenyuan_name',//分组字符串字段
                        // types : 'shequrenyuan_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "shequrenyuan_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['shequrenyuanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "shequrenyuan/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            shequrenyuanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "shequrenyuan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "shequrenyuan/update",
                        method: "post",
                        data: {
                            id:id,
//                            shequrenyuanTypes:shequrenyuanTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
